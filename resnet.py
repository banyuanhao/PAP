...
class ResNet(nn.Module):
    def __init__(self, layers, width_multiplier, sk_ratio, out_channels =1000 ):
        super().__init__()
        ...
        self.net = nn.Sequential(*ops)
        self.fc = nn.Linear(channels_in, out_channels)
...
def get_resnet(depth=50, width_multiplier=1, sk_ratio=0,out_channels = 1000):  # sk_ratio=0.0625 is recommended
    layers = {50: [3, 4, 6, 3], 101: [3, 4, 23, 3], 152: [3, 8, 36, 3], 200: [3, 24, 36, 3]}[depth]
    resnet = ResNet(layers, width_multiplier, sk_ratio, out_channels)
