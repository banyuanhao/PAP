...
def L4A_fuse(model,args):
    ...
    for (image, _) in tqdm(dataloader, disable=args.disable_tqdm):
        ...
        if args.model_arch == 'simclr':
            ...
            handle = model.net[args.target_layer2].register_forward_hook(activation_recorder_hook)
            remove_handles.append(handle)
        else:
            handle = model.blocks[args.target_layer1].register_forward_hook(activation_recorder_hook)
            remove_handles.append(handle)
            handle = model.blocks[args.target_layer2].register_forward_hook(activation_recorder_hook)
            remove_handles.append(handle)
        ...
        loss += torch.mean(activations[0]) + torch.mean(activations[1]) * args.lamuda
        loss.backward()
