...
def std(model, args=None):
    ...
    dataloader = DataLoader(dataset, num_workers=args.workers, shuffle=True, batch_size=batch_size,
                            pin_memory=True)

    delta = torch.rand(1, 3, 224, 224, device=args.device) * 2 * bound - bound
    delta.requires_grad = True

    target_layer = 2 if args.model_arch == 'simclr' else 3
